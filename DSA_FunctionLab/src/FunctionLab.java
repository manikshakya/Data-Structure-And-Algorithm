...
public class FunctionLab{
 public static void main(String args[]){
   ...
   Function<List<Integer>, List<Integer>> getEven = ls -> {
	   ...
	   for(Integer x : ls) if(x % 2 == 0) tmp.add(x);
	   return tmp;
   };
   ...
   Predicate<List<Integer>> allPos = ls -> {
	   for(Integer x : ls) if(x <= 0) return false;
	   return true;
   };
   ...
   assert mls.contains(x -> x > 2) == false;
...
class MyListInt{
 ...
 public boolean contains(Predicate<Integer> pred){
	 for(Integer x : data) if(pred.test(x)) return true;
	 return false;
 }
 
 public List<Integer> get(Predicate<Integer> pred){
	 List<Integer> tmp = new ArrayList<>();
	 for(Integer x : data) if(pred.test(x)) tmp.add(x);
	 return tmp;
 }
